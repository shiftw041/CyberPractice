...
一般情况下, 迷宫是只有 1 个入口和 1 个出口, 像入口在最左上角`(0, 0)`位置, 而出口在最右下角`(max_X, max_Y)`处. 但也有可能是出口在迷宫的正中心, 用一个`Y`字符表示等等. 解答迷宫题的条件也是需要根据具体情况判断的.
...
![setmaze.png](https://ctf-wiki.org/reverse/maze/figure/setmaze.png)

我们可以到`.rodata`段用光标选择所有的地图字符串, 按下`shift+E`提取所有的地图数据.

![extract_maze.png](https://ctf-wiki.org/reverse/maze/figure/extract_maze.png)
...
![maze_origin.png](https://ctf-wiki.org/reverse/maze/figure/maze_origin.png)

再来看迷宫移动所需要的字符:

![choice.png](https://ctf-wiki.org/reverse/maze/figure/choice.png)

这里我们知道, 可以使用的字符有`L/R/U/D`, 分别对应于`左/右/上/下`.

再往下看

![check.png](https://ctf-wiki.org/reverse/maze/figure/check.png)

通过调试是可以知道, 这里其实是每次在用户输入`L/R/U/D`后, 先打印一次你的输入, 然后打印对应的`X/Y坐标`. 而最后的判定成功的条件, 就是当`pos_x == 89 && pos_y == 28`. 那么我们就可以根据上述信息, 获得走出迷宫的路径.

![maze.jpg](https://ctf-wiki.org/reverse/maze/figure/maze.jpg)
...
[攻防世界逆向入门题之maze](https://blog.csdn.net/xiao__1bai/article/details/119765825?spm=1001.2101.3001.6650.4&utm_medium=distribute.pc_relevant.none-task-blog-2%7Edefault%7EBlogCommendFromBaidu%7ERate-4-119765825-blog-121411564.235%5Ev43%5Epc_blog_bottom_relevance_base8&depth_1-utm_source=distribute.pc_relevant.none-task-blog-2%7Edefault%7EBlogCommendFromBaidu%7ERate-4-119765825-blog-121411564.235%5Ev43%5Epc_blog_bottom_relevance_base8&utm_relevant_index=9)
